...
| `pretooluse-inline-script-write-gate.ts`       | Blocks `node -e` and `bun -e` / `--eval` inline scripts that contain file-write API calls (`writeFile`, `appendFile`, `createWriteStream`, `Bun.write`, and their sync variants). Inline eval scripts bypass the native Write/Edit tool review path — use the Write or Edit tools to make file changes instead.                                                                                                                                                                                                                                                                                                    |
...
| `pretooluse-apply-rsc-gate.ts`                 | Requires the `/apply-rsc` skill to be invoked before editing Next.js App Router page (`**/app/**/page.tsx`) or layout (`**/layout.tsx`) files. Skipped when the skill is not installed. Enforces correct Server/Client Component boundaries and import conventions.                                                                                                                                                                                                                                                                                                                                               |
