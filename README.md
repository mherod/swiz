...
| `pretooluse-peer-staged-path-deletion.ts`       | Blocks `trash <path>` and `git rm [--cached] <path>` when `git status` reports the path staged in the index (`A`/`AD`/`M`/`R`/`C`) — a deliberate act by whoever staged it, and in a shared checkout possibly another live session's in-flight work. Single-path scoping is not evidence of safety, which is why the wildcard guards miss this shape. Fails open outside a repository or when Git cannot answer.                                                                                                                                                                                                                       |
