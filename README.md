...
| `pretooluse-require-convert-to-kotlin.ts`     | Requires the `/convert-to-kotlin` skill to be invoked before editing Java (`*.java`) files in projects where both Gradle and Kotlin have been detected. Enforces migrating Java files to Kotlin to maintain codebase consistency. |
