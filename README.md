...
| `pretooluse-guardian-awareness.ts`             | Detects Codex shell calls that proactively request guardian review. Requires an ordinary sandboxed attempt first, keeps successful sandboxed operations non-escalated even when they emit incidental permission warnings, and permits a narrowly scoped escalation only after the transcript proves a concrete sandbox permission or network failure. A confirmed `git add` index-lock failure is handled specially: the hook denies escalation and steers isolated tracked-file work to `git commit -a`; untracked or mixed changes must remain uncommitted. |
