...
| `pretooluse-infraction-escalation.ts`          | Escalates a graduated consequence for retry-after-block: re-issuing an Edit, Write, or Bash call that a PreToolUse hook already denied, instead of doing what the block asked. The first retry of a blocked action gets a yellow-card advisory; a second-or-later retry is hard-blocked (red card) and directs the agent to take the originally-required action or use `/re-assess`. It never fires on the first block or on healthy-but-improvable advisory state — only on a concrete, already-denied action being repeated. Pure transcript scan, no state files. |
