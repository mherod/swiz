...
| `stop-ship-checklist.ts`         | **Unified ship gate** — combines git sync (commit / pull / push), GitHub CI on feature branches in peer-review collaboration mode (polls up to 30s for in-flight runs), and actionable issues/PRs. Respects `gitStatusGate`, `githubCiGate`, and `personalRepoIssuesGate` independently. Emits one preamble and one ordered action plan so the agent sees a single checklist, not three separate blocks. |
