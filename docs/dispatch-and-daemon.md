...
6. **Capture** — sanitized capture envelopes are scheduled without blocking daemon forwarding; the daemon owns the single per-event content-free JSONL summary append. Nested credentials are redacted, `_env` values become key names, and normalization is stored as a delta rather than a second payload copy. Exact wire bytes are disabled by default and require `SWIZ_CAPTURE_INCOMING_RAW=1`. Disable all capture with `SWIZ_CAPTURE_INCOMING=0`; tune age, total bytes, and JSONL segment bytes with `SWIZ_INCOMING_RETENTION_MS`, `SWIZ_INCOMING_MAX_BYTES`, and `SWIZ_INCOMING_JSONL_MAX_BYTES`.
...
- **Captures**: `/tmp/swiz-incoming/` — recent sanitized envelopes and rotated per-event JSONL segments, stored in a `0700` directory with `0600` files. Parse-error envelopes contain only byte count and SHA-256 metadata. Exact `.raw.json` companions appear only with `SWIZ_CAPTURE_INCOMING_RAW=1`. Throttled cleanup runs outside request handling and compacts active JSONL records by `_capturedAt`; defaults are a 10-minute age window, 64 MiB total, and 4 MiB per JSONL segment. Inspect with `bun src/dispatch/incoming-inspect.ts list-events --json`.
