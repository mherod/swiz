...
async function runHookScript(file: string): Promise<{ exitCode: number | null; stdout: string; stderr: string }> {
  ...
  await writeFile(transcriptPath, JSON.stringify({ type: "user", message: { content: "hello" } }) + "\n");
...
describe("hook scripts contracts", () => {
  const hookFiles = [...new Set(manifest.flatMap((group) => group.hooks.map((hook) => hook.file)))].sort();
