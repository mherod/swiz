...
async function getOpenPRsWithFeedback(cwd: string, currentUser: string): Promise<number> {
  const output = await gh([
    "pr", "list", "--state", "open", "--author", currentUser,
    "--json", "number,reviewDecision",
    "--jq", 'map(select(.reviewDecision == "CHANGES_REQUESTED" or .reviewDecision == "REVIEW_REQUIRED")) | length',
  ], cwd);
...
async function main(): Promise<void> {
  try {
    ...
    if (prCount > 0) {
      ...
      reasonLines.push(`You have ${prCount} open PR(s) with pending feedback (CHANGES_REQUESTED or REVIEW_REQUIRED).`);
      reasonLines.push(skillAdvice(
        "work-on-prs",
