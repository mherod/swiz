#!/usr/bin/env bun
// Stop hook: Block stop if debug statements found in recently committed files

import { blockStop, git, isGitRepo, SOURCE_EXT_RE, TEST_FILE_RE } from "./hook-utils.ts"
import { stopHookInputSchema } from "./schemas.ts"

// CLI and hook infrastructure uses console.log as its output channel — not debugging
export const INFRA_FILE_RE =
  /hooks\/|\/commands\/|\/tasks\/|\/dispatch\/|\/cli\.|index\.ts$|dispatch\.ts$|(?:^|\/)scripts\//

// Compiled/generated artifacts contain machine-written console calls — not authored debug statements
export const GENERATED_FILE_RE = /main\.dart\.js$|\.dart\.js$|\.min\.js$|\.bundle\.js$|\.chunk\.js$/

// Config files reference debug-related rule names (e.g. "no-debugger", "no-console") — not debug statements
export const CONFIG_FILE_RE =
  /(?:^|\/)\.[a-z]+rc\.(js|mjs|cjs|ts)$|\.config\.(js|mjs|cjs|ts)$|(?:^|\/)\.eslintrc(\.json)?$/

// Debug patterns per language
export const JS_DEBUG_RE = /\bconsole\.(log|debug|trace|dir|table)\b/
export const JS_COMMENT_RE = /\/\/.*console\./
export const DEBUGGER_RE = /\bdebugger\b/
export const ESLINT_DEBUGGER_RULE_RE = /no-debugger/
export const PY_PRINT_RE = /\bprint\s*\(/
export const PY_EXCLUDE_RE = /# noqa|# debug ok/i
export const RUBY_DEBUG_RE = /\b(?:binding\.pry|byebug)\b/

function isDebugStatement(content: string): boolean {
  if (JS_DEBUG_RE.test(content) && !JS_COMMENT_RE.test(content)) return true
  if (DEBUGGER_RE.test(content) && !ESLINT_DEBUGGER_RULE_RE.test(content)) return true
  if (PY_PRINT_RE.test(content) && !PY_EXCLUDE_RE.test(content)) return true
  if (RUBY_DEBUG_RE.test(content)) return true
  return false
}

function scanDiffForDebugStatements(diff: string, limit = 15): string[] {
  const findings: string[] = []
  for (const line of diff.split("\n")) {
    if (!line.startsWith("+") || line.startsWith("+++")) continue
    if (isDebugStatement(line.slice(1))) findings.push(line.slice(0, 150))
    if (findings.length >= limit) break
  }
  return findings
}

function isSourceFile(f: string): boolean {
  return (
    SOURCE_EXT_RE.test(f) &&
    !TEST_FILE_RE.test(f) &&
    !INFRA_FILE_RE.test(f) &&
    !GENERATED_FILE_RE.test(f) &&
    !CONFIG_FILE_RE.test(f)
  )
}

async function findDebugFindings(cwd: string): Promise<string[]> {
  const GIT_EMPTY_TREE = "4b825dc642cb6eb9a060e54bf8d69288fbee4904"
  const base = (await git(["rev-parse", "--verify", "HEAD~10"], cwd)) || GIT_EMPTY_TREE
  const range = `${base}..HEAD`

  const changedRaw = await git(["diff", "--name-only", range], cwd)
  if (!changedRaw) return []

  const sourceFiles = changedRaw.split("\n").filter(isSourceFile)
  if (sourceFiles.length === 0) return []

  const diff = await git(["diff", range, "--", ...sourceFiles], cwd)
  if (!diff) return []

  return scanDiffForDebugStatements(diff)
}

function formatDebugReason(findings: string[]): string {
  let reason = "Debug statements found in recently committed source files.\n\n"
  reason += `Occurrences (${findings.length}):\n`
  for (const f of findings) reason += `  ${f}\n`
  reason +=
    "\nRemove debug statements before stopping. If intentional logging is needed, use a proper logger (e.g., pino, winston) instead."
  return reason
}

async function main(): Promise<void> {
  const input = stopHookInputSchema.parse(await Bun.stdin.json())
  const cwd = input.cwd ?? process.cwd()

  if (!(await isGitRepo(cwd))) return

  const findings = await findDebugFindings(cwd)
  if (findings.length === 0) return

  blockStop(formatDebugReason(findings), { includeUpdateMemoryAdvice: false })
}

if (import.meta.main) void main()
