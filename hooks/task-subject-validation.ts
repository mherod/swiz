...
const ACTION_VERBS = /^(add|fix|update|remove|delete|create|refactor|migrate|implement|improve|rename|move|extract|replace|rewrite|enable|disable|configure|set up|clean up|write|deploy|build|generate|publish|merge|revert|bump|review|approve|reject|audit|profile|optimize|benchmark|document|analyse|analyze)\b/i;
...
export function detect(s: string): DetectionResult {
  ...
  if ((s.match(/,/g) ?? []).length >= 2) {
    ...
    const trailingAreSuffixes = parts.slice(1).every(
      (p) => p.trim().length <= 20 && !ACTION_VERBS.test(p.trim())
    );
    ...
    if (lastVerbMatch) {
      ...
      const leadingAreBareVerbs = sharedObject.length > 0 &&
        parts.slice(0, -1).every((p) => ACTION_VERBS.test(p.trim()) && p.trim().split(/\s+/).length <= 2);
