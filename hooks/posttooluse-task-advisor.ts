...
async function main(): Promise<void> {
  ...
  if (callsSinceTask >= total) {
    ...
    if (remaining <= 1) {
      emit(`TaskCreate required in ${remaining} tool call(s) — tools will be blocked until tasks are defined.`);
    } else if (remaining <= 3) {
      emit(`TaskCreate required in ${remaining} tool calls. Plan your tasks now to avoid interruption.`);
    } else if (total >= 2) {
  ...
  if (staleRemaining <= 2) {
    emit(`Task update required in ${staleRemaining} tool call(s) — tools will be blocked until tasks are reviewed.`);
  } else if (staleRemaining <= 4) {
    emit(`Task update due in ${staleRemaining} tool calls. Review progress — mark completed tasks done or create new ones.`);
  }
}
