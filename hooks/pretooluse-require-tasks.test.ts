...
describe("pretooluse-require-tasks", () => {
  ...
  test("allows Edit when canonical TaskList sync is missing and task minimums are met", async () => {
    const homeDir = await createTempHome()
    const sessionId = `session-missing-tasklist-sync-edit-${Date.now()}`
    await writeTask(homeDir, sessionId, {
      id: "1",
      subject: "Active task",
      status: "in_progress",
  ...
  test("allows Edit when canonical TaskList sync is older than 20 minutes", async () => {
    const homeDir = await createTempHome()
  ...
  test("still denies Bash when canonical TaskList sync is older than 20 minutes", async () => {
    const homeDir = await createTempHome()
    const sessionId = `session-stale-tasklist-sync-bash-${Date.now()}`
    await writeTask(homeDir, sessionId, {
      id: "1",
      subject: "Active task",
      status: "in_progress",
...
describe("task-file block bypass regression tests", () => {
  ...
  test("relaxes pending task requirement when auto-continue is disabled (0 pending allowed)", async () => {
    const homeDir = await createTempHome()
    ...
    const sessionId = "session-relaxed-autocontinue-false"
  ...
  test("enforces pending task requirement when auto-continue is enabled (1 pending required)", async () => {
    const homeDir = await createTempHome()
    await mkdir(join(homeDir, ".swiz"), { recursive: true })
