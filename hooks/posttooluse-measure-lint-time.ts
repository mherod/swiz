  ...
  if (await file.exists()) {
    try {
      ...
      if (typeof data.startTime === "number") {
        ...
        const statsPath = join(projectRoot, ".swiz", "lint-execution-stats.json")
        ...
        return {
          systemMessage: `Lint run took ${currentSeconds.toFixed(2)}s. Average lint execution time updated: ${averageSeconds.toFixed(2)}s (based on ${stats.count} runs).`,
        }
      }
    } catch {
      // Non-fatal
