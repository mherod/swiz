...
- **Hook tone**: Human voice; next action first. DON'T mention Swiz/hook mechanics, audit/sync/cache/drift, raw counts, thresholds, or echo detector patterns. Scold behavior, direct next action.
...
- **Last-task-standing enforcement**: `updateStatus()` calls unconditional `validateLastTaskStanding`; on empty-would-result, completion is blocked until a successor task is created explicitly. `skipLastTaskGuard` for explicit overrides.
