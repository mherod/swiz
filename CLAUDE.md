...
**DO** complete the "Push and verify CI" task using the CLI with explicit CI evidence: `bun ~/.claude/hooks/tasks-list.ts --complete <id> "Push and verify" --evidence "note:CI green — conclusion: success, run <run-id>"`. The stop-completion-auditor checks for CI verification evidence on completed tasks — using `TaskUpdate` alone (without `--evidence`) leaves the task without evidence and triggers a stop block.
...
- **DON'T use `awk` in Bash commands.** A PreToolUse hook blocks `awk` invocations. For file content analysis, use `bun -e` with a TypeScript one-liner. For file modifications, use the Edit tool.
