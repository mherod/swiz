...
- **Stop hook advice footers re-trigger memory enforcement**: The stop-git-status, stop-auto-continue, and similar stop hooks append a memory-advice footer containing the REMINDER_FRAGMENT. Each time a stop hook fires (e.g., because of uncommitted changes), it creates a new enforcement trigger. The gate must be satisfied again (skill read + markdown write) before Bash/Edit will unblock. A planned mitigation is to add a cooldown check based on the CLAUDE.md modified time — if CLAUDE.md was written within the last N minutes, skip the enforcement gate.
