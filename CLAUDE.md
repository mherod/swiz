...
- **DON'T edit files outside the session sandbox.** The `pretooluse-sandboxed-edits` hook blocks Edit/Write to paths outside the session's CWD repository. Files in `~/.claude/hooks/` belong to the `mherod/.claude` repo — to modify them, either switch to that repo's session or file an issue with `gh issue create --repo mherod/.claude`. Skills in `~/.claude/skills/` belong to `mherod/skills` — file issues there instead. When filing cross-repo implementation requests, include: problem statement, desired solution, specific implementation location (file path and line numbers/section name), validation logic or examples, and success criteria. Example: filed `mherod/skills#44` to request label validation in refine-issue skill — included problem (missing validation), solution (validation before ready marking), location (before "## Refinement Checklist" section), validation logic (3 required label categories), and success criteria.
