...
- **DO**: Treat **CI verification and native-task closure as part of the same workflow block as push**, not only when a stop hook lists “wait for CI” / “mark tasks completed”: right after `git push origin main`, run `SHA=$(git rev-parse HEAD)`, `gh run list --repo mherod/swiz --commit "$SHA" --limit 1 --json databaseId,status,conclusion`, then `gh run view <databaseId> --repo mherod/swiz --json conclusion,status,jobs` (and `gh run view <id> --log-failed` on non-success). In that same closure window, **TaskUpdate** / `swiz tasks complete` every open session task tied to the work, using evidence prefixes **`commit:`**, **`test:`**, **`file:`**, or **`note:`** only ([swiz#300](https://github.com/mherod/swiz/issues/300) documents rejected keys).
