...
import { AGENTS, getAgentByFlag, type AgentDef } from "../agents.ts";
  ...
  if (obj && typeof obj === "object") {
    ...
    if (Array.isArray(rec.hooks)) {
      const filtered = rec.hooks.filter((h) => {
        ...
        return !(
          typeof hh.command === "string" && hh.command.includes(HOOKS_DIR)
        );
      });
...
async function uninstallAgent(agent: AgentDef, dryRun: boolean) {
  ...
  const cleaned = cleanEmptyEvents(
    removeSwizHooks(hooks) as Record<string, unknown[]>
  );
  ...
  console.log(
    `  ${BOLD}${agent.name}${RESET}: ${RED}removing ${oldCount} swiz hook(s)${RESET}`
  );
