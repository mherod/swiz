...
export const cleanupCommand: Command = {
  ...
  async run(args: string[]) {
    ...
    for (const { name, keep, old } of results) {
      ...
      console.log(`    ${name.padEnd(44)} ${String(total).padStart(3)} sessions  →  ${keep.length} kept, ${trashPart}`);
    }
    ...
    console.log(`  Total: ${BOLD}${totalOldCount} sessions${RESET} trashable, ~${formatBytes(totalOldBytes)}`);
    console.log();
    ...
    if (failed > 0) {
      console.log(`  ${YELLOW}${failed} session(s) could not be trashed — is the \`trash\` CLI installed?${RESET}`);
    }
  },
};
