...
describe("typed task store keys", () => {
  ...
  it("keeps both kinds in the flat store, told apart by metadata rather than by path", async () => {
    ...
    expect((await readdir(base)).sort()).toEqual([projectKeyFromCwd(cwd), "abcd-session"].sort())
  })
