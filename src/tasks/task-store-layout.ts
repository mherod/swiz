...
/**
 * Fold a namespaced project store into its flat home.
 *
 * Used when both directories exist, which a plain rename cannot resolve. Task
 * records move only into free names, so neither copy of a colliding record is
 * overwritten. `.session-meta.json` is the one
