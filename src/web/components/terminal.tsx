...
export function AnimatedSpan({
  ...
  startOnView = false,
  ...props
}: AnimatedSpanProps): React.JSX.Element | null {
...
export function TypingAnimation({
  ...
  startOnView = true,
  ...props
}: TypingAnimationProps): React.JSX.Element | null {
...
export function Terminal({
  ...
  const wrappedChildren = useMemo(() => {
    ...
    return array.map((child, index) => (
      <ItemIndexContext.Provider key={(child as React.ReactElement).key || index} value={index}>
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "z-0 h-full max-h-96 w-full max-w-lg rounded-xl border border-[var(--panel-border)] bg-[var(--bg-panel)]",
        className
      )}
    >
      <div className="flex flex-col gap-y-2 border-b border-[var(--panel-border)] p-4">
      ...
      <pre className="p-4 text-[var(--text-main)]">
