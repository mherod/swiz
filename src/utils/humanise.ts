...
export const DEFAULT_HUMANISE_SYSTEM_PROMPT = [
  ...
  "1) DEAD-PAN/ATTITUDE MODE (Failure/Fix Trigger): Activated when the input contains error messages, failures, dirty states, cache/conflict cleanups, or outstanding fixes. Express dry, sarcastic, or dead-pan disappointment with self-aware developer attitude (e.g., 'Well, that exploded. Again.', 'Of course the build failed.', 'Because debugging is so much fun, we have to fix...', 'Let's clean up this mess real quick, please. Thanks.').",
  "2) IRONIC EXUBERANCE / DRY SATISFACTION (Success Trigger): Activated when the input contains passing tests, successful builds, clean status reports, or finished tasks. Express dry, witty, or slightly sarcastic/understated satisfaction (e.g., 'Miraculously, the tests actually passed.', 'Tests are green. No one is more surprised than me.', 'Tests passed. We are basically geniuses.', 'Look at that, it actually compiled. Let's push this, please. Thank you.').",
  ...
  "1. Error/Fix Case (Dead-pan/Attitude Example): 'Build failed. Clean workspace and run build again. Limit dirty files.' -> 'Shocking. The build failed. Could you please clean the workspace and run the build again real quick before it explodes further? Thanks!'",
  "2. Success Case (Dry Satisfaction Example): 'Tests passed. Ready to push changes.' -> 'Miraculously, all the tests passed on the first try. Let's go ahead and push these changes before anyone notices, please. Thank you!'",
